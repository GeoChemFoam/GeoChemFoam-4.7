...
print('Value of the pores is:'+np.str(pores_value))

img = np.fromfile(target_direc+image_name+'.raw', dtype='uint8', sep="")
img = np.reshape(img, (z_dim, y_dim, x_dim))
img2=np.zeros((x_dim,y_dim,z_dim),dtype='uint8')
for i in range(0,x_dim):
    for j in range(0,y_dim):
        for k in range(0,z_dim):
            img2[i,j,k]=img[k,j,i]
img_crop = img2[x_min-1:x_max+1,y_min-1:y_max+1,z_min-1:z_max+1]
img_padded = np.pad(img_crop, pad_width=1,mode='constant', constant_values=pores_value)

print('Unique values in the padded image are:'+np.str(np.unique(img_padded)))



pores = (img_padded==pores_value)*1
distance_map = ndimage.distance_transform_edt(pores)
search_size=np.int(min((x_max-x_min)/4,(y_max-y_min)/4,(z_max-z_min)/4))
...
verts, faces, normals, values = skimage.measure.marching_cubes_lewiner(img_padded, (pores_value+solid_value)/2, step_size=1, allow_degenerate=1)
